...
function groupJsArg(v) {
  if (typeof jsArg === "function") return jsArg(v);
  return groupEsc(JSON.stringify(String(v ?? "")));
}
...
function renderGroupsAdmin(S, createGroup, deleteGroup) {
  ...
  Object.entries(groups).forEach(([code, grp]) => {
    ...
    o += `<div>`;
    o += `<div style="font-size:14px;font-weight:700">${groupEsc(grp.name)}</div>`;
    o += `<div style="font-size:12px;color:var(--t3)">Código: <b style="color:var(--pl)">${groupEsc(code)}</b> · ${memberCount} miembro${memberCount!==1?"s":""}</div>`;
    if (grp.prize) o += `<div style="font-size:11px;color:var(--gd);margin-top:2px">🏆 1°: ${groupEsc(grp.prize)}</div>`;
    if (grp.prize_last) o += `<div style="font-size:11px;color:var(--rd);margin-top:1px">💀 Último: ${groupEsc(grp.prize_last)}</div>`;
    o += `</div>`;
    o += `<button style="background:rgba(239,68,68,.12);border:none;border-radius:8px;padding:6px 10px;color:var(--rd);font-size:11px;font-weight:700;cursor:pointer" onclick="if(confirm(${groupJsArg("¿Borrar grupo "+code+"?")}))deleteGroup(${groupJsArg(code)})">🗑️</button>`;
    o += `</div>`;
    ...
    if (members.length) {
      ...
      members.forEach(([name]) => {
        o += `<span style="font-size:11px;background:var(--c2);border-radius:6px;padding:2px 8px;color:var(--t2)">👤 ${groupEsc(name)}</span>`;
      });
      o += `</div>`;
    }
    o += `</div>`;
...
function renderJoinGroup(S, playerName, joinGroup, leaveGroup) {
  ...
  if (myGroups.length) {
    myGroups.forEach(code => {
      ...
      o += `<div><span style="font-size:12px;font-weight:600">${groupEsc(grp.name)}</span>`;
      if (grp.prize) o += `<span style="font-size:10px;color:var(--gd);margin-left:6px">🏆 ${groupEsc(grp.prize)}</span>`;
      if (grp.prize_last) o += `<span style="font-size:10px;color:var(--rd);margin-left:4px">💀 ${groupEsc(grp.prize_last)}</span>`;
      o += `</div>`;
      o += `<button style="background:none;border:none;color:var(--rd);font-size:11px;cursor:pointer;font-weight:600" onclick="leaveGroup(${groupJsArg(code)})">Salir</button>`;
      o += `</div>`;
    });
  } else {
...
function renderGroupRankings(S, myGroups, calcFn, compFn, flag) {
  ...
  myGroups.forEach(code => {
    ...
    o += `<div style="font-size:14px;font-weight:800;margin-bottom:2px">👥 ${groupEsc(grp.name)}</div>`;
    if (grp.prize) o += `<div style="font-size:11px;color:var(--gd);margin-bottom:8px">🏆 ${groupEsc(grp.prize)}</div>`;
    ...
    members.forEach((r, pos) => {
      ...
      o += `<div style="flex:1;font-size:13px;font-weight:${isMe?800:600}">${groupEsc(r.n)}${isMe?' <span style="font-size:10px;color:var(--pl)">(vos)</span>':""}</div>`;
